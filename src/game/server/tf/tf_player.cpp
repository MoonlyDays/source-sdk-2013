...
void CTFPlayer::ManageRegularWeapons(TFPlayerClassData_t* pData)
{
	...
	if (iClass > TF_CLASS_UNDEFINED && iClass < TF_CLASS_COUNT)
	{
		...
		for (int i = 0; i < CLASS_LOADOUT_POSITION_COUNT; i++)
		{
			...
			{
				auto pNewItem = dynamic_cast<CEconEntity*>(GiveNamedItem(pItem->GetStaticData()->GetItemClass(), 0, pItem));
				Assert(pNewItem);
				if (pNewItem)
				{
					pNewItem->GetAttributeContainer()->GetItem()->SetOverrideAccountID(ownerSteamID.GetAccountID());

					auto pBuilder = dynamic_cast<CTFWeaponBuilder*>(static_cast<CBaseEntity*>(pNewItem));
					if (pBuilder)
					{
						pBuilder->SetSubType(pData->m_aBuildable[0]);
					}

					auto pWeapon = dynamic_cast<CBaseCombatWeapon*>(pNewItem);
					if (pWeapon)
					{
					...
					m_Shared.SetItemChargeMeter(static_cast<loadout_positions_t>(i), pNewItem->GetDefaultItemChargeMeterValue());
...
//-----------------------------------------------------------------------------
void CTFPlayer::UseActionSlotItemPressed()
{
...
CBaseEntity* CTFPlayer::GiveNamedItem(const char* pszName, int iSubType, const CEconItemView* pScriptItem, bool bForce)
{
	...
	auto pWeapon = dynamic_cast<CBaseCombatWeapon*>(pItem);
	if (pWeapon)
	{
