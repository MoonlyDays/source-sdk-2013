...
void CTFBot::EquipBestWeaponForThreat(const CKnownEntity* threat)
{
	...
	case TF_CLASS_ENGINEER:
	{
		// const auto mySentry = dynamic_cast<CObjectSentrygun*>(GetObjectOfType(OBJ_SENTRYGUN));
		// if (mySentry && !mySentry->IsBuilding() && !mySentry->IsUpgrading())
		{
			auto pWrangler = dynamic_cast<CTFWeaponBase*>(GetEntityForLoadoutSlot(LOADOUT_POSITION_SECONDARY));
			if (pWrangler /*&& IsRangeGreaterThan(threat->GetLastKnownPosition(), SENTRY_MAX_RANGE)*/)
			{
				gun = pWrangler;
			}
		}
