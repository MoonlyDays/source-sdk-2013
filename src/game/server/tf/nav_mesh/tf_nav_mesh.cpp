...
void CTFNavMesh::ComputeIncursionDistances(void)
{
	...
	FOR_EACH_VEC(TheNavAreas, it)
	{
		auto area = dynamic_cast<CTFNavArea*>(TheNavAreas[it]);
	...
	{
		const auto spawnRoom = dynamic_cast<CFuncRespawnRoom*>(IFuncRespawnRoomAutoList::AutoList()[i]);
		...
		{
			auto spawnSpot = dynamic_cast<CTFTeamSpawn*>(ITFTeamSpawnAutoList::AutoList()[i]);
			...
			if (spawnRoom->PointIsWithin(spawnSpot->GetAbsOrigin()))
			{
				...
				auto spawnArea = dynamic_cast<CTFNavArea*>(TheTFNavMesh()->GetNearestNavArea(spawnSpot));
				if (spawnArea)
				{
	...
	if (!TFGameRules()->IsMannVsMachineMode())
	{
		...
		for (int i = 0; i < TheNavAreas.Count(); ++i)
		{
			auto area = dynamic_cast<CTFNavArea*>(TheNavAreas[i]);
